...
public class MudoStatusEffect extends StatusEffect {
    ...
    public MudoStatusEffect() {
        super(StatusEffectCategory.HARMFUL, 0x551A8B); // 紫色っぽい色
    }
    ...
    public void applyUpdateEffect(LivingEntity entity, int amplifier) {
        if (!entity.getWorld().isClient) {
            ...
            double maxReduction = Math.min(level, 18); // 上限：10
            ...
            if (entity.getHealth() > currentMax) {
                entity.setHealth((float) currentMax);
            }
        }
    }
